...
def _strip_markup(text: str) -> str:
    """Прибирає Textual/Rich markup теги для звичайного тексту в статусі."""
    return re.sub(r"\[/?[^\]]+\]", "", text)


def resolve_input_path(raw_path: str, project_dir: Path) -> Path:
    """
    Розв'язує шлях до вхідного файлу.
    - Якщо вказано абсолютний шлях і файл існує — використовуємо його.
    - Інакше шукаємо відносно папки проекту (наприклад input/teacher-01.wav).
    - Шлях на кшталт /project_01/input/file.wav сприймається як input/file.wav відносно project_dir.
    """
    ...
    # Відносно робочої папки проекту
    normalized = raw_path.replace("\\", "/").lstrip("/")
    if normalized.lower().startswith("project_01/"):
        normalized = normalized[11:]  # прибрати префікс project_01/
    candidate = project_dir / normalized
    if candidate.exists():
        return candidate.resolve()

    return p  # повертаємо як є — далі буде зрозуміла помилка "файл не знайдено"
...
class DubbingApp(App):
    ...
    BINDINGS = [
        ...
        Binding("y", "copy_log", "Копіювати лог"),
    ]
    ...
    def __init__(self, config: AppConfig | None = None, **kwargs):
        ...
        self._log_lines: list[str] = []  # plain-text рядки для копіювання
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            ...
            with VerticalScroll(id="left-panel"):
                ...
                yield Input(
                    placeholder="input/teacher-01.wav (або повний шлях)",
                    id="input-file",
                )
            ...
            with Vertical(id="right-panel"):
                ...
                yield Label("Поточний стан:", classes="section-title")
                yield Label("Готовий до запуску.", id="status-label")
                yield Label("Історія логу:", classes="section-title")
    ...
    def on_mount(self) -> None:
        ...
        self._set_status("Готовий до запуску.")
    ...
    def _handle_run(self) -> None:
        raw_path = self.query_one("#input-file", Input).value.strip()
        if not raw_path:
            self._log("[bold red]Помилка:[/bold red] Вкажіть шлях до файлу!")
            return

        file_path = resolve_input_path(raw_path, PROJECT_DIR)
        if not file_path.exists():
            self._log(
                f"[bold red]Файл не знайдено:[/bold red] {raw_path}\n"
                f"[dim]Підказка: вкажіть відносно папки проекту, напр. input/teacher-01.wav[/dim]"
            )
            return
    ...
    def _log(self, message: str) -> None:
        """
        Запис у RichLog та оновлення «Поточний стан» зверху.
        ...
        try:
            ...
            rich_log.write(message, scroll_end=True)
            # Показуємо останнє повідомлення у блоці «Поточний стан»
            status_text = plain or "—"
    ...
    def action_copy_log(self) -> None:
        """Копіює весь plain-text лог у буфер обміну Windows (clip.exe)."""
        if not self._log_lines:
            self._log("[dim]Лог порожній — нічого копіювати.[/dim]")
            return
        ...
        try:
            ...
            self._log("[dim]Лог скопійовано в буфер обміну.[/dim]")
        except Exception as e:
            self._log(f"[yellow]Не вдалось скопіювати: {e}[/yellow]")
